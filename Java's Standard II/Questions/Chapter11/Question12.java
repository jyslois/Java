...
getPoint()를 완성하시오.
[참고] 섯다게임은 두 장의 카드의 숫자를 더한 값을 10으로 나눈 나머지가 높은 쪽이
이기는 게임이다. 그 외에 도 특정 숫자로 구성된 카드로 이루어진 등급(족보)이 있어서
높은 등급의 카드가 이긴다.
...
class SutdaDeck {
    ...
    SutddaCard[] cards = new SutddaCard[CARD_NUM];
    ...
    SutdaDeck() {
        for(int i=0; i < cards.length; i++) {
            ...
            cards[i] = new SutddaCard(num, isKwang);
        }
...
*/
    ...
    int getPoint(Player p) {
        ...
        // (c1.num + c2.num) % 10 + 1000
            if (result == null) {   // get메서드의 호출결과가 null이면 족보에 없다는 것을 의미
    ...
    SutddaCard pick() throws Exception {
        SutddaCard c = null;
...
class Player {
    ...
    Player(String name, SutddaCard c1, SutddaCard c2) {
        this.name = name ;
        this.c1 = c1 ;
        this.c2 = c2 ;
    }
...
class SutddaCard {
    ...
    SutddaCard(int num, boolean isKwang) {
        this.num = num;
